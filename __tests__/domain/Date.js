...
describe('Date/#validate 생성 예외테스트', () => {
  test.each(['월요일', 'one', ' ', '$$', '3.14'])(
    '날짜가 숫자 형식이어야 한다. 그렇지 않으면 DateTypeError를 반환해야 한다.',
    input => {
  ...
  test.each(['-1', '0', '32', '40', '50'])(
    '날짜가 1이상 31이하여야 한다. 그렇지 않으면 DateTypeError를 반환해야 한다.',
    input => {
      expect(() => new Date(input)).toThrowError(DateTypeError);
    },
  );
...
describe('Date/hasStar 별 표시 날짜 반환 테스트', () => {
...
describe('Date/isWeekend 주말 여부 반환 테스트', () => {
